...
% % JBZ edits
% %                maskCQ    = griddata(grd.lon_rho,grd.lat_rho,grd.mask_rho,xCQ,yCQ,'nearest');
% % Put all coordinates for all points in Nx2 matrix (lon,lat)
% if iT == tinxStart  %find interpolation triangle once for all times/levels
%    matsize = size(grd.lon_rho,1) .* size(grd.lon_rho,2);
%    romsgrd(:,1) = reshape(grd.lon_rho,1,matsize);
%    romsgrd(:,2) = reshape(grd.lat_rho,1,matsize);
%    romsmask(:,1) = reshape(grd.mask_rho,1,matsize);
%    tri = delaunayn(romsgrd);
% end  %end iT = tinxStart
%    maskCQ = tinterp(romsgrd,tri,romsmask,xCQ,yCQ);
%      % Masking produces some NaNs, set those to "land points"
%      maskCQ(isnan(maskCQ))=0;
% % End JBZ edits
...
%         5,L0_h,squeeze(L0_zeta(:,:,tt)));
%     
%     L0_z(isnan(L0_z)==1)=0;
%     
%     for zz=1:L0_N
%         L0_Hz(:,:,zz)=L0_z(:,:,zz+1)-L0_z(:,:,zz);
%     end
%     
%     L0_u=double(ncread(L0_out,'u',[L0_xiniu,L0_yiniu,1,L0_tini+tt-1],[L0_nxu,L0_nyu,inf,1]));
%     L0_v=double(ncread(L0_out,'v',[L0_xiniv,L0_yiniv,1,L0_tini+tt-1],[L0_nxv,L0_nyv,inf,1]));
%     [L0_ubar(:,:),L0_vbar(:,:)]=uv_barotropic(squeeze(L0_u(:,:,:,1)),squeeze(L0_v(:,:,:,1)),squeeze(L0_Hz(:,:,:)));
%     
%     clear L0_Hz
